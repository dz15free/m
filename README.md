...
- [x] Phase 15 — العمل دون إنترنت (عامل خدمة يخزّن واجهة التطبيق وصفحة «غير متصل»، مؤشر الاتصال والمزامنة)؛ إشعارات الهاتف عبر Firebase Cloud Messaging: إعلانات الإدارة لكل الأجهزة حسب اللغة، رد الإدارة على رسالة الأستاذ، تفعيل الاشتراك
- [x] استيراد جدول التوقيت من Excel أو PDF أو صورة (أيام في الأسطر أو الأعمدة، صيغ الأوقات الشائعة، مكوّنات المواد، الأقسام «1م2»، دمج الحصص المزدوجة) مع مراجعة قبل الحفظ ونموذج Excel جاهز؛ قراءة قوائم الرقمنة (PDF وصورة) بدقة
- [x] Phase 16 — الصقل: مراجعة أمنية (الخروج يمسح بيانات التلاميذ المخزّنة على الجهاز، ملفات المكتبة بأنواع مسموحة فقط وفي sandbox، روابط الإدارة الآمنة، CSP/HSTS/COOP)، صفحات الخطأ و404، صفحتا شروط الاستخدام وسياسة الخصوصية (القانون 18-07)، تحسين صور الشعار (srcset)

### إشعارات الهاتف (مرة واحدة)
Firebase Console → Project settings → Cloud Messaging → **Web Push certificates** → Generate key pair، ثم ضع **المفتاح العام** في `.env.production`:
`NEXT_PUBLIC_FIREBASE_VAPID_KEY=<المفتاح العام>` — قيمة عامة وليست سرًّا. بدونها تبقى بطاقة «فعّل الإشعارات» مخفية، ويعمل كل شيء آخر.
الإرسال يتم من الخادم بحساب الخدمة (`FIREBASE_SERVICE_ACCOUNT`)؛ على iPhone تعمل الإشعارات بعد تثبيت التطبيق على الشاشة الرئيسية (iOS 16.4+).
